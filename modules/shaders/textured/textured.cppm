...
export module ce.shaders.textured;
import ce.vk;
import ce.vk.shader;
import glm;
...
{
class TexturedShader final : public vk::ShaderModule
{
public:
    #include "textured.h"
private:
    bool create_layout()
    {
        ...
        if (const VkResult result = vkCreateDescriptorSetLayout(m_vk.device(), &set_info[2],
            nullptr, &m_set_layouts[2]); result != VK_SUCCESS)
        {
            return false;
        }
        m_vk.debug_name(std::format("{}-MaterialSetLayout", m_name), m_set_layouts[2]);
    ...
    bool create_pools(const uint32_t pools_count, const uint32_t frame_sets, const uint32_t object_sets,
        const uint32_t material_sets) noexcept
    {
        ...
        for (size_t i = 0; i < pools_count; i++)
        {
            const std::array descr_pool_sizes{
                ...
                VkDescriptorPoolSize{ VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, material_sets },
            };
            const VkDescriptorPoolCreateInfo descr_pool_info{
                .sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO,
                .maxSets = frame_sets + object_sets + material_sets,
...
public:
    explicit TexturedShader(const std::shared_ptr<vk::Context>& vk, const std::string_view name)
        : ShaderModule(vk, std::format("Textured-{}", name)) { }
    ~TexturedShader() noexcept override = default;
    bool create(VkRenderPass renderpass, const uint32_t pools_count, const VkSampleCountFlagBits sample_count,
        const uint32_t frame_sets, const uint32_t object_sets, const uint32_t material_sets,
        const bool enable_blending, const bool double_sided, const bool wireframe) noexcept
    {
        if (!load_from_file("assets/shaders/textured-vs.spv", "assets/shaders/textured-ps.spv") ||
            !create_layout() || !create_pipeline(renderpass, sample_count, enable_blending, double_sided, wireframe) ||
            !create_pools(pools_count, frame_sets, object_sets, material_sets))
        {
            return false;
        }
        return true;
