...
export module ce.shaders.raymarch;
import ce.vk;
import ce.vk.shader;
import glm;
...
{
class RaymarchShader final : public vk::ShaderModule
{
public:
    #include "raymarch.h"
private:
...
public:
    explicit RaymarchShader(const std::shared_ptr<vk::Context>& vk, const std::string_view name)
        : ShaderModule(vk, std::format("Raymarch-{}", name)) { }
    ~RaymarchShader() noexcept override = default;
    ...
    {
        if (!load_from_file("assets/shaders/raymarch-vs.spv", "assets/shaders/raymarch-ps.spv") ||
