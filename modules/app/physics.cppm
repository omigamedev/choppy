...
constexpr JPH::ObjectLayer LAYER_NUM_LAYERS = 2;
constexpr JPH::BroadPhaseLayer BROAD_NON_MOVING(0);
constexpr JPH::BroadPhaseLayer BROAD_MOVING(1);
constexpr JPH::uint BROAD_NUM_LAYERS(2);
...
public:
	bool ShouldCollide(JPH::ObjectLayer inObject1, JPH::ObjectLayer inObject2) const override
	{
		...
		case LAYER_NON_MOVING:
			return inObject2 == LAYER_MOVING; // Non moving only collides with moving
		case LAYER_MOVING:
...
public:
    BPLayerInterfaceImpl()
	{
		...
		mObjectToBroadPhase[LAYER_NON_MOVING] = BROAD_NON_MOVING;
		mObjectToBroadPhase[LAYER_MOVING] = BROAD_MOVING;
	}

	JPH::uint GetNumBroadPhaseLayers() const override
	{
		return BROAD_NUM_LAYERS;
	}

	JPH::BroadPhaseLayer GetBroadPhaseLayer(JPH::ObjectLayer inLayer) const override
	{
		JPH_ASSERT(inLayer < LAYER_NUM_LAYERS);
	...
	const char* GetBroadPhaseLayerName(JPH::BroadPhaseLayer inLayer) const override
	{
		...
		{
		case (JPH::BroadPhaseLayer::Type)BroadPhaseLayers::BROAD_NON_MOVING:	return "NON_MOVING";
		case (JPH::BroadPhaseLayer::Type)BroadPhaseLayers::BROAD_MOVING:		return "MOVING";
...
private:
	JPH::BroadPhaseLayer mObjectToBroadPhase[LAYER_NUM_LAYERS];
};
...
public:
	...
	JPH::Ref<JPH::Character> create_character() noexcept
	{
	    ...
	    settings->mLayer = LAYER_MOVING; // Your custom object layer for players
	    settings->mUp = JPH::Vec3::sAxisY(); // Gravity direction is always Y-axis for Jolt characters

	    // Optional settings:
	    // settings->mMaxStrength = 100.0f;
	    settings->mFriction = 0.0f;
	    JPH::Ref character = new JPH::Character(settings, {10, 10, 10}, JPH::Quat::sIdentity(), 0, &physics_system);
	...
	std::optional<std::tuple<JPH::BodyID, JPH::RefConst<JPH::Shape>>> create_chunk_body(
	    const uint32_t chunk_size, const float block_size, const ChunkData& data, const glm::ivec3 sector) noexcept
	{
	    // Create physics body
